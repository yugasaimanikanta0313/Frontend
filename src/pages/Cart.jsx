...
const cleanImageUrl = (url) => {
  ...
  prefixes.forEach((prefix) => {
    if (cleanedUrl.startsWith(prefix)) {
      cleanedUrl = cleanedUrl.slice(prefix.length);
    }
  });
  ...
  try {
    const amount = calculateTotal() * 100; // 💡 converting rupees to paise
    const userId = getSession("userId");
    ...
    const options = {
      ...
      amount: data.amount, // should already be in paise from backend
      currency: "INR",
      name: "Agro Bazaar",
                ...
                {cartItems.map((item) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <img
                        src={cleanImageUrl(item.product.pictureUrl1)}
